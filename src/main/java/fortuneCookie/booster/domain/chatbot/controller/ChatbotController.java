...
import fortuneCookie.booster.domain.chatbot.service.ChatService;
...
public class ChatbotController {
    ...
    private static final String prompt = """
            ...
            **이전 대화 내역:**
            ...
            답변 규칙:
            1. 이전 대화 내용을 참고하여 연속적인 대화가 가능하도록 답변하세요.
            2. {nickname}님이라고 친근하게 불러주세요.
            3. "그것", "그거", "마감일" 등의 애매한 표현도 이전 대화 맥락에서 이해하세요.
            4. 정확한 답변을 위해 문서 정보만을 바탕으로 답변하세요.
            5. 정보가 부족하거나 찾을 수 없다면, "해당 정보를 찾을 수 없습니다. 교무팀에 직접 문의해주세요"라고 답변하세요.
            6. 답변은 한국어로, 존댓말을 사용하여 친근하게 해주세요.
            7. 중요한 날짜나 마감일이 있다면 강조해서 알려주세요.
    ...
    public ResponseEntity<Map<String, Object>> askQuestion(@RequestBody Map<String, Object> request,
        ...
        try {
            ...
            if (sessionId == null || sessionId.trim().isEmpty()) {
                ...
                log.info("새로운 채팅 세션 시작 - 사용자: {}, 세션: {}", nickname, sessionId);
            } else {
                log.info("기존 채팅 세션 계속 - 사용자: {}, 세션: {}", nickname, sessionId);
            }

            log.info("을지대학교 챗봇 질문 수신: {}", question);

            //이전 대화 조회
            List<Chat> previousChats = chatService.getRecentChats(userId, sessionId, 10);
            String chatHistory = chatService.buildChatContext(previousChats);

            if (previousChats.isEmpty()) {
                log.info("첫 번째 질문입니다.");
            } else {
                log.info("연속 질문입니다. 이전 대화 {}개 참조", previousChats.size());
            }

            // 사용자 질문 저장
            chatService.saveUserMessage(userId, sessionId, question);
            ...
            promptParameters.put("chatHistory", chatHistory);

            promptParameters.put("nickname", nickname);
            ...
            // 챗봇 답변 저장
            chatService.saveAssistantMessage(userId, sessionId, answer);
